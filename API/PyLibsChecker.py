...
def IIL(ln="", lfn="", im=True,sP='Lib/site-packages',MN='QOAI',Rs=False):
	try:
		...
		if im == True:
			...
			IOS = subprocess.Popen([os.path.join(pyPath,'python'), "-m", "pip", "install", f"{exMessage.name if lfn == '' else lfn}"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			...
			if IOS.returncode == 0:
				...
				print(f"Could not installation library: {exMessage}")
